  ...
  return (
    ...
    >
          ...
          {isMainRow ? (
            ...
            <div className={cn("flex items-center gap-2", "pl-2 sm:pl-4")}>
              ...
              <span className={cn(
                ...
                {displayPhone}
                {isCallback && (
                  <span className={cn("text-[10px] sm:text-xs ml-1", "text-gray-500 dark:text-gray-400")}>
        ...
        {isOutgoing ? (
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm">
              <span className="text-gray-900 dark:text-gray-100">{call.city?.name || '—'}</span>
      ...
      <TableCell className="py-2 sm:py-3 px-2 sm:px-4">
        <div className="space-y-0.5">
          <div className="flex flex-col sm:flex-row sm:items-center sm:gap-1.5 text-xs sm:text-sm">
            {false && <Clock className="w-3.5 h-3.5 flex-shrink-0 text-gray-500 hidden sm:block" />}
            <span className={cn("hidden sm:inline", "text-gray-600 dark:text-gray-400")}>{formatDate(call.createdAt)}</span>
            <span className={cn("font-medium", "text-gray-900 dark:text-gray-100")}>{formatTime(call.createdAt)}</span>
            <span className={cn("sm:hidden text-[10px]", "text-gray-500 dark:text-gray-400")}>{formatDate(call.createdAt)}</span>
          </div>
          {duration && (
            <div className={cn("text-[10px] sm:text-xs font-mono", "text-gray-500 dark:text-gray-400")}>
      ...
      <TableCell className="py-2 sm:py-3 px-2 sm:px-4">
        <div className="space-y-1">
          <div className="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm">
            <User className={cn("w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0", "text-gray-400")} />
            <span className="text-gray-900 dark:text-gray-100">{call.operator.name}</span>
          </div>
          <div className="flex items-center gap-1 flex-wrap">
            ...
            >
              <DirectionIcon className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
              <span className="hidden sm:inline">{directionConfig.label}</span>
            </Badge>
            {/* Статус звонка */}
            <Badge 
              variant="outline" 
