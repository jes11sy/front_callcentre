...
function PenaltiesContent() {
  ...
  if (isLoading && !user) {
    return (
      <div className={`flex items-center justify-center h-64 ${isV2 ? 'bg-[#F3F3EE]' : ''}`}>
        <div className="text-center">
          <Loader2 className={`h-8 w-8 animate-spin mx-auto mb-4 ${isV2 ? 'text-[#FEC004]' : 'text-[#FFD700]'}`} />
          <p className={isV2 ? 'text-gray-500' : 'text-gray-400'}>Загрузка данных пользователя...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <DashboardLayout>
      <div className={`w-full py-4 px-4 min-h-screen custom-scrollbar ${isV2 ? 'bg-[#F3F3EE] font-myriad' : 'bg-[#0f0f23]'}`}>
...
function PenaltiesFallback() {
  ...
  return (
    <div className={`flex items-center justify-center h-screen ${isV2 ? 'bg-[#F3F3EE]' : 'bg-[#0f0f23]'}`}>
