  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-1">
            {onEdit && (
              <Button
                ...
                onClick={onEdit}
                className={isV2 
                  ? "h-8 w-8 sm:w-auto sm:px-2 p-0 sm:p-0 text-gray-500 dark:text-gray-400 hover:text-[#FEC004] hover:bg-[#FEC004]/10"
                  : "h-8 w-8 sm:w-auto sm:px-2 p-0 sm:p-0 text-gray-400 hover:text-[#FFD700] hover:bg-[#FFD700]/10"
                }
              >
        ...
        {/* Tabs */}
        <div className={isV2 
          ? "flex border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-[#252d3a] px-1 sm:px-2 overflow-x-auto"
          : "flex border-b border-[#FFD700]/30 bg-[#17212b]/50 px-1 sm:px-2 overflow-x-auto"
        }>
          <button
            onClick={() => setActiveTab('info')}
            className={`px-3 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-medium transition-colors whitespace-nowrap ${
              activeTab === 'info'
                ? (isV2 ? 'text-[#FEC004] border-b-2 border-[#FEC004] -mb-px' : 'text-[#FFD700] border-b-2 border-[#FFD700] -mb-px')
                : (isV2 ? 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200' : 'text-gray-400 hover:text-gray-200')
            }`}
          >
            Информация
          </button>
...
const InfoTab = ({ 
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 sm:divide-x ${isV2 ? 'divide-gray-200 dark:divide-gray-700' : 'divide-[#FFD700]/20'}`}>
        ...
        <div className="p-3 sm:p-4 space-y-1.5 sm:space-y-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2 sm:mb-3">
            <h3 className={`text-xs sm:text-sm font-medium ${isV2 ? 'text-gray-900 dark:text-gray-100' : 'text-[#FFD700]'}`}>Информация по заказу</h3>
            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
              <Badge 
                className={`text-[10px] sm:text-xs ${statusColors[order.statusOrder as keyof typeof statusColors] || (isV2 ? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300' : 'bg-gray-800 text-gray-300')}`}
              >
        ...
        <div className={`p-3 sm:p-4 space-y-1.5 sm:space-y-2 border-t sm:border-t-0 ${isV2 ? 'border-gray-200 dark:border-gray-700' : 'border-[#FFD700]/20'}`}>
          <h3 className={`text-xs sm:text-sm font-medium ${isV2 ? 'text-gray-900 dark:text-gray-100' : 'text-[#FFD700]'} mb-2 sm:mb-3`}>Мастер и финансы</h3>
      ...
      {/* Оператор */}
      <div className={isV2 
        ? "px-3 sm:px-4 py-2 sm:py-3 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-[#252d3a] flex items-center"
        : "px-3 sm:px-4 py-2 sm:py-3 border-t border-[#FFD700]/20 bg-[#17212b]/30 flex items-center justify-between"
      }>
        <div className="flex items-center gap-1.5 sm:gap-2">
          <span className={`text-xs sm:text-sm ${isV2 ? 'text-gray-500 dark:text-gray-400' : 'text-gray-400'}`}>Оператор:</span>
          <span className={`text-xs sm:text-sm ${isV2 ? 'text-gray-900 dark:text-gray-100' : 'text-white'}`}>{order.operator.name}</span>
        </div>
        {!isV2 && <span className="text-[10px] sm:text-xs text-gray-500">ID: {order.operatorNameId}</span>}
      </div>

      {/* Записи звонков */}
      <div className={`px-3 sm:px-4 py-2 sm:py-3 border-t ${isV2 ? 'border-gray-200 dark:border-gray-700' : 'border-[#FFD700]/20'}`}>
        <h3 className={`text-xs sm:text-sm font-medium ${isV2 ? 'text-gray-900 dark:text-gray-100' : 'text-[#FFD700]'} mb-2 sm:mb-3`}>Записи звонков</h3>
        
        {loadingCalls ? (
          <div className={isV2 
            ? "flex items-center gap-2 p-2 sm:p-3 bg-white dark:bg-[#252d3a] rounded-lg border border-gray-200 dark:border-gray-700"
            : "flex items-center gap-2 p-2 sm:p-3 bg-[#17212b] rounded-lg border border-[#FFD700]/20"
          }>
            <LoadingSpinner size="sm" />
            <span className={`text-xs sm:text-sm ${isV2 ? 'text-gray-500 dark:text-gray-400' : 'text-gray-400'}`}>Загрузка...</span>
          </div>
        ) : orderCalls.length > 0 ? (
          ...
          <div className={isV2 
            ? "p-2 sm:p-3 bg-white dark:bg-[#252d3a] rounded-lg border border-gray-200 dark:border-gray-700 text-center"
            : "p-2 sm:p-3 bg-[#17212b] rounded-lg border border-[#FFD700]/20 text-center"
          }>
            <span className={`text-xs sm:text-sm ${isV2 ? 'text-gray-400 dark:text-gray-500' : 'text-gray-500'}`}>Записи не найдены</span>
          </div>
        )}
      </div>
    </div>
...
const DataRow = ({ label, value, muted = false, isV2 = false }: { label: string; value: string; muted?: boolean; isV2?: boolean }) => (
  <div className="flex justify-between items-start gap-2">
    <span className={`text-[10px] sm:text-xs ${isV2 ? 'text-gray-400 dark:text-gray-500' : 'text-gray-500'} shrink-0`}>{label}</span>
    <span className={`text-xs sm:text-sm text-right break-words min-w-0 ${muted ? (isV2 ? 'text-gray-400 dark:text-gray-500' : 'text-gray-500') : (isV2 ? 'text-gray-900 dark:text-gray-100' : 'text-white')}`}>{value}</span>
  </div>
);
...
const CallPlayer = ({
  ...
  return (
    <div className={isV2 
      ? "p-2 sm:p-3 bg-white dark:bg-[#252d3a] rounded-lg border border-gray-200 dark:border-gray-700"
      : "p-2 sm:p-3 bg-[#17212b] rounded-lg border border-[#FFD700]/20"
    }>
      <div className="flex items-center justify-between gap-2">
        <div className="flex flex-col sm:flex-row sm:items-center gap-0.5 sm:gap-3 min-w-0">
          <span className={`text-xs sm:text-sm font-medium ${isV2 ? 'text-gray-900 dark:text-gray-100' : 'text-white'}`}>Звонок #{call.id}</span>
          {callDate && (
            <span className={`text-[10px] sm:text-xs ${isV2 ? 'text-gray-400 dark:text-gray-500' : 'text-gray-500'}`}>
        ...
        <Button
          ...
          className={`shrink-0 ${isV2 
            ? "h-7 w-7 sm:h-8 sm:w-8 p-0 text-gray-400 hover:text-[#FEC004] hover:bg-[#FEC004]/10"
            : "h-7 w-7 sm:h-8 sm:w-8 p-0 text-gray-400 hover:text-[#FFD700] hover:bg-[#FFD700]/10"
          }`}
